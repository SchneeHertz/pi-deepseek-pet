...
配套的 Windows 桌面应用负责展示桌宠。[从 GitHub Releases 下载最新安装器](https://github.com/SchneeHertz/pi-deepseek-pet/releases/latest) 并安装。可手动启动；也可在桌面应用设置中启用 **随 Pi 启动和退出**，让本扩展在 Pi 启动时自动拉起桌宠，并在最后一个 Pi 进程退出后关闭。
